...
class Matoa(IStrategy):
    ...
    buy_max_volatility2 = DecimalParameter(0.5, 2, default=1, decimals=2, space='buy',
                                           optimize=optimize_buy_volatility2)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not self.optimize_buy_hma2:
            dataframe['hma_offset_buy2'] = tv_hma(dataframe,
                                                  int(self.base_nb_candles_buy_hma2.value)) * self.low_offset_hma2.value

        if not self.optimize_buy_hma3:
            dataframe['hma_offset_buy3'] = tv_hma(dataframe,
                                                  int(self.base_nb_candles_buy_hma3.value)) * self.low_offset_hma3.value
        ...
        if not self.optimize_buy_volatility2:
            ...
            dataframe["volatility2"] = (df_std > self.buy_min_volatility.value) & (
                        df_std < self.buy_max_volatility2.value)
